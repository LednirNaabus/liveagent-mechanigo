...
async def extract_and_load_tickets_initial(date: pd.Timestamp, table_name: str):
    """
    This function is used for initial full load of fetching all tickets.
    """
    filters = set_filter(date, filter_field=FilterField.DATE_CREATED)
    ticket_payload = {
        "_perPage": 10,
        "_sortDir": "DESC",
        "_filters": filters
    ...
    async with LiveAgentClient(config.API_KEY) as client:
        ...
        try:
            if success:
                ...
                file_name = os.path.join('csv', f"tickets-{date}.csv")
...
async def extract_and_load_tickets_incremental(date: pd.Timestamp, table_name: str):
    """
    This function is for incremental or daily fetching of tickets.
    """
    filters = set_filter(date, filter_field=FilterField.DATE_CHANGED)
    ticket_payload = {
        "_perPage": 10,
        "_filters": filters
    }
